...
@else
    <div class="grid gap-6" style="grid-template-columns:1fr 360px;align-items:start;">
        ...
        <div class="bg-white border border-fs-border rounded-[2px] p-7"
             x-data="{ paymentType: '{{ old('payment_type', $customer->default_payment_type ?? 'Visa') }}' }">
                ...
                <div class="mb-5">
                    ...
                    <textarea name="address"
                              ...
                              class="w-full bg-white border rounded-[6px] px-3 py-2.5 text-fs-dark text-[0.9rem] outline-none box-border resize-y
                                     ...
                                     focus:border-fs-purple">{{ old('address', $customer->address) }}</textarea>
                    @error('address')
                        <p class="text-red-400 text-[0.73rem] mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-col gap-2.5 mb-4">
                @foreach($items as $item)
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-[0.82rem] font-semibold {{ $item['has_discount'] ? 'text-[#4ade80]' : 'text-fs-purple' }}">
