<div>
    {{-- Flash --}}
    @if(session('success'))
        <div style="background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.2);color:#4ade80;padding:0.75rem;border-radius:6px;margin-bottom:1.25rem;font-size:0.85rem;">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div>
        <div style="background:#111120;border:1px solid #1e1e30;border-radius:12px;padding:1.75rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:1.25rem;">
                ...
                {{-- Unit price catalog --}}
                <div>
                    <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Preço unitário — catálogo (€)</label>
                ...
                {{-- Unit price own --}}
                <div>
                    <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Preço unitário — própria (€)</label>
                ...
                <div>
                    <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Preço com desconto — catálogo (€)</label>
                ...
                {{-- Own discount price --}}
                <div>
                    <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Preço com desconto — própria (€)</label>
            ...
            <div style="margin-bottom:1.75rem;">
                <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Quantidade mínima para desconto (unidades)</label>
                <input wire:model="qtyDiscount" type="number" step="1" min="1"
                    style="background:#1a1a2e;border:1px solid #1e1e30;color:#e2e8f0;border-radius:6px;padding:0.6rem 0.75rem;width:100%;max-width:200px;box-sizing:border-box;font-size:0.9rem;outline:none;" />
                @error('qtyDiscount') <div style="color:#ef4444;font-size:0.75rem;margin-top:0.3rem;">{{ $message }}</div> @enderror
                <p style="color:#64748b;font-size:0.78rem;margin:0.4rem 0 0;">A partir desta quantidade, os preços com desconto são aplicados automaticamente.</p>
            </div>

            {{-- Save --}}
            <div style="border-top:1px solid #1e1e30;padding-top:1.25rem;">
                <button wire:click="save" wire:loading.attr="disabled"
                    style="background:#7c3aed;color:white;border:none;border-radius:6px;padding:0.6rem 1.5rem;cursor:pointer;font-size:0.9rem;font-weight:600;">
