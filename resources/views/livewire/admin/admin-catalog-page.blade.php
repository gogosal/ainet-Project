<div>
    ...
    <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1.25rem;flex-wrap:wrap;">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Pesquisar por nome…"
            style="background:#ffffff;border:1px solid #e0ddd8;color:#1a1a1a;border-radius:1px;padding:0.6rem 0.75rem;width:220px;box-sizing:border-box;font-size:0.9rem;outline:none;" />

        <select wire:model.live="categoryFilter"
            style="background:#ffffff;border:1px solid #e0ddd8;color:#1a1a1a;border-radius:1px;padding:0.6rem 0.75rem;font-size:0.9rem;outline:none;">
        ...
        <button wire:click="openCreate"
            style="background:#7c6fa0;color:white;border:none;border-radius:1px;padding:0.55rem 1.1rem;cursor:pointer;font-size:0.85rem;font-weight:600;margin-left:auto;">
            + Adicionar imagem
        </button>
    </div>
    ...
    {{-- Grid --}}
    @if($images->isEmpty())
        <div style="text-align:center;color:#888;padding:3rem;background:#ffffff;border:1px solid #e0ddd8;border-radius:2px;">
            Nenhuma imagem encontrada.
        </div>
    @else
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:1rem;margin-bottom:1.5rem;">
            @foreach($images as $img)
                <div style="background:#ffffff;border:1px solid #e0ddd8;border-radius:2px;overflow:hidden;display:flex;flex-direction:column;">
                    <div style="height:160px;background:#f5f4f1;display:flex;align-items:center;justify-content:center;overflow:hidden;">
                        @if($img->image_url)
                            @php
                                $catalogImgUrl = str_contains($img->image_url, '/')
                    ...
                    <div style="padding:0.75rem;flex:1;display:flex;flex-direction:column;gap:0.3rem;">
                        <div style="color:#1a1a1a;font-size:0.9rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $img->name }}</div>
                        <div style="color:#888;font-size:0.75rem;">
                            {{ $img->category?->name ?? '—' }}
                        </div>
                        @if($img->description)
                            <div style="color:#aaa;font-size:0.75rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $img->description }}</div>
                        @endif
                        <div style="display:flex;gap:0.5rem;margin-top:auto;padding-top:0.5rem;">
                            <button wire:click="openEdit({{ $img->id }})"
                                style="background:#ffffff;color:#888;border:1px solid #e0ddd8;border-radius:1px;padding:0.4rem 0.75rem;cursor:pointer;font-size:0.8rem;flex:1;">
                                Editar
                            </button>
    ...
    @if($showModal)
        <div style="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:50;display:flex;align-items:center;justify-content:center;padding:1rem;">
            <div style="background:#ffffff;border:1px solid #e0ddd8;border-radius:2px;width:100%;max-width:480px;padding:1.5rem;max-height:90vh;overflow-y:auto;">
                ...
                <div style="display:flex;flex-direction:column;gap:1rem;">
                    ...
                    {{-- Description --}}
                    <div>
                        <label style="color:#888;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Descrição</label>
                        <textarea wire:model="modalDescription" rows="3" placeholder="Descrição opcional"
                            style="background:#ffffff;border:1px solid #e0ddd8;color:#1a1a1a;border-radius:1px;padding:0.6rem 0.75rem;width:100%;box-sizing:border-box;font-size:0.9rem;outline:none;resize:vertical;"></textarea>
                        @error('modalDescription') <div style="color:#ef4444;font-size:0.75rem;margin-top:0.3rem;">{{ $message }}</div> @enderror
                    </div>

                    {{-- Category --}}
                    <div>
                        <label style="color:#888;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Categoria</label>
    ...
    @if($deleteId)
        <div style="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:50;display:flex;align-items:center;justify-content:center;padding:1rem;">
            <div style="background:#ffffff;border:1px solid #e0ddd8;border-radius:2px;width:100%;max-width:380px;padding:1.5rem;">
                ...
                <p style="color:#888;font-size:0.875rem;margin:0 0 1.25rem;">Tem a certeza que deseja eliminar esta imagem? Esta ação é irreversível.</p>
