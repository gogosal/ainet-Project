<div>
    ...
    @if (session('success'))
        <div
            style="background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.2);color:#4ade80;padding:0.75rem;border-radius:6px;margin-bottom:1rem;font-size:0.85rem;">
            {{ session('success') }}
        </div>
    @endif
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="border-bottom:1px solid #1e1e30;">
                    <th
                        style="color:#94a3b8;font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;padding:0.75rem 1rem;text-align:left;">
                        Cor</th>
                    <th
                        style="color:#94a3b8;font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;padding:0.75rem 1rem;text-align:left;">
                        Nome</th>
                    ...
                    <th
                        ...
                        Imagem base</th>
                    <th
                        style="color:#94a3b8;font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;padding:0.75rem 1rem;text-align:right;">
                        Ações</th>
                </tr>
            </thead>
            <tbody>
                @forelse($colors as $color)
                    ...
                    <tr style="border-bottom:1px solid #1e1e30;">
                        <td style="padding:0.75rem 1rem;">
                            <div
                                style="width:40px;height:40px;border-radius:8px;background:{{ $color->code }};border:2px solid rgba(255,255,255,.1);">
                            </div>
                        </td>
                        <td style="padding:0.75rem 1rem;color:#e2e8f0;font-size:0.9rem;font-weight:500;">
                            {{ $color->name }}</td>
                        <td style="padding:0.75rem 1rem;">
                            <span
                                style="background:#1a1a2e;border:1px solid #1e1e30;color:#a78bfa;border-radius:4px;padding:0.2rem 0.5rem;font-size:0.78rem;font-family:monospace;">{{ $color->code }}</span>
                        </td>
                        <td style="padding:0.75rem 1rem;">
    ...
    @if ($showModal)
        <div
            ...
            <div
                style="background:#111120;border:1px solid #1e1e30;border-radius:12px;width:100%;max-width:420px;padding:1.5rem;">
                ...
                <div style="display:flex;flex-direction:column;gap:1rem;">
                    ...
                    @if (!$editingCode)
                        <div>
                            <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Código hex
                                *</label>
                    ...
                    @else
                        <div>
                            <label
                                style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Código</label>
                            <div style="display:flex;gap:0.5rem;align-items:center;">
                                <div
                                    style="width:36px;height:36px;border-radius:6px;background:{{ $modalCode }};border:2px solid rgba(255,255,255,.1);flex-shrink:0;">
                                </div>
                                <span
                                    style="background:#1a1a2e;border:1px solid #1e1e30;color:#94a3b8;border-radius:6px;padding:0.6rem 0.75rem;font-family:monospace;font-size:0.9rem;">{{ $modalCode }}</span>
                            </div>
                        </div>
                    @endif
                    ...
                    {{-- Base image --}}
                    <div>
                        <label style="color:#94a3b8;font-size:0.8rem;display:block;margin-bottom:0.4rem;">Imagem base da
                        ...
                        @if ($modalImage)
                            <div style="margin-top:0.5rem;">
                                <img src="{{ $modalImage->temporaryUrl() }}"
                                    style="max-height:80px;border-radius:4px;border:1px solid #1e1e30;" />
                            </div>
                        @endif
                    </div>
                </div>
    ...
    @if ($deleteCode)
        <div
            style="position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:50;display:flex;align-items:center;justify-content:center;padding:1rem;">
            <div
                style="background:#111120;border:1px solid #1e1e30;border-radius:12px;width:100%;max-width:380px;padding:1.5rem;">
                <h2 style="color:#e2e8f0;font-size:1rem;font-weight:700;margin:0 0 0.75rem;">Confirmar eliminação</h2>
                <p style="color:#94a3b8;font-size:0.875rem;margin:0 0 1.25rem;">Tem a certeza que deseja eliminar a cor
                    <strong style="color:#e2e8f0;">{{ $deleteCode }}</strong>? Esta ação é irreversível.</p>
