<div style="display:flex;flex-direction:column;height:calc(100vh - 64px - 4rem);">
    ...
    <div style="display:grid;grid-template-columns:230px 1fr 260px;gap:1.25rem;flex:1;min-height:0;">
        ...
        {{-- RIGHT: Controls --}}
        <div style="background:#111120;border:1px solid #1e1e30;border-radius:12px;padding:0.85rem;display:flex;flex-direction:column;gap:0.6rem;">
            @if($selectedImage)
                <div style="border-bottom:1px solid #1e1e30;padding-bottom:0.55rem;">
                    <p style="color:#a78bfa;font-size:0.68rem;font-weight:600;margin:0 0 0.2rem;text-transform:uppercase;letter-spacing:.05em;">Design</p>
                    <p style="color:#e2e8f0;font-size:0.85rem;font-weight:600;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $selectedImage->name }}</p>
                </div>
            @endif

            <div>
                <p style="color:#a78bfa;font-size:0.68rem;font-weight:600;margin:0 0 0.35rem;text-transform:uppercase;letter-spacing:.05em;">Cor</p>
                ...
                @if($selectedColor)
                    ...
                    <p style="color:#64748b;font-size:0.72rem;margin:0.2rem 0 0;">{{ $colorName ?? $selectedColor }}</p>
                @endif
            </div>

            <div>
                <p style="color:#a78bfa;font-size:0.68rem;font-weight:600;margin:0 0 0.35rem;text-transform:uppercase;letter-spacing:.05em;">Tamanho</p>
            ...
            <div>
                <p style="color:#a78bfa;font-size:0.68rem;font-weight:600;margin:0 0 0.35rem;text-transform:uppercase;letter-spacing:.05em;">Quantidade</p>
                <div style="display:flex;align-items:center;gap:0.4rem;">
                    ...
                    <span style="color:#e2e8f0;font-weight:600;min-width:1.8rem;text-align:center;font-size:0.9rem;">{{ $qty }}</span>
            ...
            @if($prices)
                <div style="background:#1a1a2e;border-radius:7px;padding:0.55rem 0.65rem;">
                    <div style="display:flex;justify-content:space-between;color:#94a3b8;font-size:0.78rem;margin-bottom:0.2rem;">
                        <span>Preço/un</span>
                        <span style="color:#a78bfa;font-weight:600;">€{{ number_format($prices->unit_price_catalog, 2) }}</span>
                    </div>
                    @if($qty >= $prices->qty_discount)
                        <div style="display:flex;justify-content:space-between;color:#4ade80;font-size:0.72rem;margin-bottom:0.2rem;">
                    ...
                    <div style="display:flex;justify-content:space-between;color:#e2e8f0;font-weight:700;font-size:0.95rem;">
                        <span>Total</span>
                        <span style="color:#a78bfa;">€{{ number_format(($qty >= $prices->qty_discount ? $prices->unit_price_catalog_discount : $prices->unit_price_catalog) * $qty, 2) }}</span>
                    </div>
                </div>
            @endif
    ...
    (function () {
        ...
        function buildShirt() {
            ...
            // Design decal
            const designImg = currentDesignId && loadedImages[currentDesignId] ? loadedImages[currentDesignId] : null;
            if (designImg && designImg.complete && designImg.naturalWidth > 0) {
                ...
                const decal = new THREE.Mesh(
                    ...
                    new THREE.MeshStandardMaterial({ map: tex, transparent: true, alphaTest: 0.04, depthWrite: false, roughness: 0.84, polygonOffset: true, polygonOffsetFactor: -1 })
                );
