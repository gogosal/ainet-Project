<div style="max-width:1280px; margin:2.5rem auto; padding:0 1.5rem;">
    ...
    @if (session('success'))
        <div
            style="background:#14532d; border:1px solid #16a34a; color:#86efac; padding:0.85rem 1.25rem; border-radius:0.5rem; margin-bottom:1.5rem; font-size:0.9rem;">
            {{ session('success') }}
        </div>
    @endif
    ...
    @if ($images->count() > 0)
        <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:1.25rem; margin-bottom:2rem;">
            @foreach ($images as $image)
                <div
                    style="background:#111120; border:1px solid #1e1e30; border-radius:0.75rem; overflow:hidden; display:flex; flex-direction:column;">
                    {{-- Thumbnail --}}
                    <div style="aspect-ratio:1; overflow:hidden; background:#0a0a0f;">
                        <img src="{{ asset('storage/' . $image->image_url) }}" alt="{{ $image->name }}"
                            style="width: 100%; height: 100%; object-fit: cover;">
                        <div
                            style="display:none; width:100%; height:100%; align-items:center; justify-content:center; color:#94a3b8; font-size:0.8rem;">
                            Sem imagem
                        </div>
                    </div>
                    ...
                    <div style="padding:0.85rem; flex:1; display:flex; flex-direction:column; gap:0.4rem;">
                        <p style="color:#e2e8f0; font-size:0.9rem; font-weight:600; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"
                            title="{{ $image->name }}">
                            {{ $image->name }}
                        </p>
                        @if ($image->category)
                        ...
                        @if ($image->description)
                            <p
                                style="color:#94a3b8; font-size:0.78rem; margin:0; overflow:hidden; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">
                                {{ $image->description }}
                            </p>
                        @endif
                        ...
                        <div style="display:flex; gap:0.5rem; margin-top:auto; padding-top:0.5rem;">
                            <button wire:click="openEdit({{ $image->id }})"
                                ...
                                onmouseover="this.style.background='#2d2d45'"
                            ...
                            <button wire:click="confirmDelete({{ $image->id }})"
                                ...
                                onmouseover="this.style.background='#2d1515'"
                                onmouseout="this.style.background='#1e1e30'">
                                Eliminar
                            </button>
                        </div>
    ...
    @else
        <div
            style="background:#111120; border:1px solid #1e1e30; border-radius:0.75rem; padding:3rem; text-align:center;">
    ...
    @if ($showModal)
        <div
            ...
            <div
                style="background:#111120; border:1px solid #1e1e30; border-radius:0.75rem; padding:2rem; width:100%; max-width:520px; max-height:90vh; overflow-y:auto;">
                ...
                <form wire:submit="saveImage">
                    ...
                    <div style="margin-bottom:1.25rem;">
                        <label
                            style="display:block; color:#94a3b8; font-size:0.82rem; font-weight:500; margin-bottom:0.4rem;">Nome
                            *</label>
                        <input type="text" wire:model="modalName"
                            ...
                            onfocus="this.style.borderColor='#7c3aed'" onblur="this.style.borderColor='#1e1e30'">
                        @error('modalName')
                            <p style="color:#f87171; font-size:0.78rem; margin-top:0.3rem; margin-bottom:0;">
                                {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div style="margin-bottom:1.25rem;">
                        <label
                            style="display:block; color:#94a3b8; font-size:0.82rem; font-weight:500; margin-bottom:0.4rem;">Descrição</label>
                    ...
                    <div style="margin-bottom:1.25rem;">
                        <label
                            style="display:block; color:#94a3b8; font-size:0.82rem; font-weight:500; margin-bottom:0.4rem;">Categoria</label>
                    ...
                    <div style="margin-bottom:1.75rem;">
                        <label
                            style="display:block; color:#94a3b8; font-size:0.82rem; font-weight:500; margin-bottom:0.4rem;">
                        ...
                        @enderror
                        @if ($modalImage)
                            <p style="color:#94a3b8; font-size:0.78rem; margin-top:0.4rem; margin-bottom:0;">Ficheiro
                                selecionado</p>
                        @endif
                    </div>
    ...
    @if ($deleteId)
        <div
            style="position:fixed; inset:0; background:rgba(0,0,0,0.75); z-index:100; display:flex; align-items:center; justify-content:center; padding:1rem;">
            <div
                style="background:#111120; border:1px solid #1e1e30; border-radius:0.75rem; padding:2rem; width:100%; max-width:400px;">
                <h2 style="color:#e2e8f0; font-size:1.1rem; font-weight:600; margin-bottom:0.75rem;">Confirmar
                    eliminação</h2>
