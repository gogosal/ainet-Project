...
<div>
    <div class="mb-8">
        ...
        <h1 class="text-[1.6rem] font-light tracking-tight text-fs-dark m-0">A tua <em class="italic font-bold">selecção.</em></h1>
    </div>
        ...
        <div class="grid gap-6 items-start" style="grid-template-columns:1fr 300px;">
            ...
            <div>
                <div class="flex justify-end mb-3">
                    <form method="POST" action="{{ route('cart.clear') }}" onsubmit="return confirm('Tens a certeza que queres limpar o carrinho?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                ...
                <div class="flex flex-col gap-3">
                    @foreach($items as $item)
                        <div class="bg-white border border-fs-border rounded-[2px] p-[1.1rem] flex gap-[1.1rem] items-start">
                            <div class="shrink-0">
                                <x-tshirt-preview
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <form method="POST" action="{{ route('cart.update', $item['index']) }}">
                                    ...
                                    <div class="grid gap-3 items-end" style="grid-template-columns:1fr 1fr auto;">
                                        <div>
                                            <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.4rem]">Cor</label>
                                        ...
                                        <div>
                                            <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.4rem]">Tamanho</label>
                                        ...
                                        <div>
                                            <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.4rem]">Qtd</label>
                                            <div class="flex items-center gap-[0.3rem]">
                                                <input type="hidden" name="qty" id="qty-{{ $item['index'] }}" value="{{ $item['qty'] }}">
                                                <button type="button"
                                                        onclick="var i=document.getElementById('qty-{{ $item['index'] }}');var v=Math.max(1,parseInt(i.value)-1);i.value=v;this.closest('form').submit();"
                                                ...
                                                <span class="text-fs-dark font-semibold min-w-[2rem] text-center text-[0.82rem]">{{ $item['qty'] }}</span>
                                                <button type="button"
                            ...
                            <div class="flex flex-col items-end gap-[0.4rem] shrink-0">
                                <span class="{{ $item['has_discount'] ? 'text-fs-green' : 'text-fs-dark' }} font-bold text-[0.95rem]">
                                    €{{ number_format($item['sub_total'], 2) }}
                                </span>
                                <span class="text-fs-muted text-[0.7rem]">{{ $item['qty'] }}× €{{ number_format($item['unit_price'], 2) }}</span>
