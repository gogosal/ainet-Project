...
<section style="margin: -2.5rem -2rem 0; border-bottom: 1px solid #e0ddd8;">
    <div style="max-width: 1280px; margin: 0 auto; padding: 0 2rem; display: grid; grid-template-columns: 1fr 1fr; min-height: calc(100vh - 56px); align-items: stretch;">
        ...
        <div id="hero-3d-wrap" style="border-left:1px solid #e0ddd8;position:relative;overflow:hidden;">
            ...
            <div style="position:absolute;bottom:1.25rem;left:50%;transform:translateX(-50%);font-size:.58rem;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:#c8c4be;pointer-events:none;white-space:nowrap;">
                ↔ Provador 3D
            </div>
        </div>
    </div>
               ...
               onmouseover="this.style.background='#f9f8f6'" onmouseout="this.style.background='#fff'">
                @php
                    ...
                    $dUrl = \Illuminate\Support\Str::startsWith($d->image_url, 'tshirt_images_private/')
                        ...
                        : (str_contains($d->image_url, '/') ? asset('storage/'.$d->image_url) : asset('storage/tshirt_images/'.$dBare));
                @endphp
                <div class="product-img-wrap" style="background:#fff;display:flex;align-items:center;justify-content:center;height:200px;border-bottom:1px solid #e0ddd8;transition:border-color .15s;padding:1.5rem;overflow:hidden;">
        ...
        <div style="background:#eeecea;height:100%;display:flex;align-items:center;justify-content:center;padding:3rem;border-left:1px solid #e0ddd8;min-height:300px;">
            @if($featuredDesigns->first())
                @php
                    ...
                    $ctaUrl = \Illuminate\Support\Str::startsWith($featuredDesigns->first()->image_url, 'tshirt_images_private/')
                        ? route('private-image', $ctaBare)
                        : (str_contains($featuredDesigns->first()->image_url, '/') ? asset('storage/'.$featuredDesigns->first()->image_url) : asset('storage/tshirt_images/'.$ctaBare));
                @endphp
                <img src="{{ $ctaUrl }}" alt="" style="max-width:200px;max-height:200px;object-fit:contain;display:block;">
            @endif
        </div>
    </div>
</section>
...
(function() {
    ...
    const key = new THREE.DirectionalLight(0xfff8f4, 1.05); key.position.set(3, 5, 4); scene.add(key);
    const fill = new THREE.DirectionalLight(0xdde8ff, 0.32); fill.position.set(-4, 2, 2); scene.add(fill);
    const rim = new THREE.DirectionalLight(0xffffff, 0.18); rim.position.set(0, 4, -5); scene.add(rim);

    const shirtGroup = new THREE.Group();
    scene.add(shirtGroup);

    let shirtMeshes = [], chestUV = null, modelBounds = null;
    ...
    new THREE.GLTFLoader().load('/tshirt/scene.gltf', function(gltf) {
        ...
        for (const frac of [0.65,0.70,0.60,0.55,0.75,0.50,0.80]) {
            const ray = new THREE.Raycaster(new THREE.Vector3(0, modelBounds.min.y + h*frac, -5), new THREE.Vector3(0,0,1));
            for (const mesh of sorted) {
                const hits = ray.intersectObject(mesh, false);
                if (hits.length > 0 && hits[0].uv) { chestUV = hits[0].uv.clone(); break; }
            }
            if (chestUV) break;
        }
    ...
    function drawDesign(ctx, cx, cy, R) {
        ...
        ctx.fillStyle = 'rgba(245,244,241,0)'; ctx.fill();

        // Thin outer ring
        ctx.beginPath(); ctx.arc(0, 0, R, 0, Math.PI * 2);
        ctx.strokeStyle = '#1a1a1a'; ctx.lineWidth = R * 0.05; ctx.stroke();
        ...
        // thin divider line
        ctx.beginPath();
        ctx.moveTo(-R * 0.4, R * 0.26); ctx.lineTo(R * 0.4, R * 0.26);
    ...
    function applyDesign() {
        ...
        const tex = new THREE.CanvasTexture(cv); tex.flipY = true;
