            ...
            <form method="GET" action="{{ route('admin.customers') }}" class="flex gap-4 flex-wrap items-center w-full">
                ...
                <div class="flex gap-2">
                    @foreach(['all' => 'Todos', 'active' => 'Ativos', 'blocked' => 'Bloqueados'] as $value => $label)
                        <a
                            href="{{ route('admin.customers', array_merge(request()->except('filter', 'page'), ['filter' => $value, 'search' => $search ?? ''])) }}"
                            class="rounded-[1px] px-4 py-2 text-[0.85rem] font-semibold border border-fs-border no-underline {{ ($filter ?? 'all') === $value ? 'bg-fs-purple text-white border-fs-purple' : 'bg-white text-fs-gray hover:text-fs-dark' }}"
                        >{{ $label }}</a>
                    @endforeach
                </div>
            </form>
        ...
        <table class="w-full border-collapse">
            <thead>
                <tr class="bg-[#f9f8f6]">
                    ...
                    <th class="px-4 py-3 text-left text-fs-gray text-[0.8rem] font-semibold uppercase tracking-[0.05em] border-b border-fs-border">NIF</th>
            ...
            <tbody>
                @forelse ($users as $user)
                    <tr class="{{ $user->blocked ? 'bg-red-50/40' : '' }}">
                        <td class="px-4 py-3 border-b border-fs-border text-fs-dark text-[0.85rem]">
                            <div class="flex items-center gap-2">
                                @if ($user->photo_url ?? null)
                                    ...
                                    <img src="{{ $photoSrc }}" alt="" class="w-7 h-7 rounded-full object-cover">
                                @else
                                    <div class="w-7 h-7 rounded-full bg-white flex items-center justify-center text-fs-purple text-[0.75rem] font-bold">
                        ...
                        <td class="px-4 py-3 border-b border-fs-border text-fs-gray text-[0.85rem]">{{ $user->email }}</td>
                        ...
                        <td class="px-4 py-3 border-b border-fs-border text-fs-gray text-[0.85rem]">{{ $user->customer?->nif ?? '—' }}</td>
                        <td class="px-4 py-3 border-b border-fs-border text-[0.85rem]">
                            @if ($user->blocked)
                                <span class="bg-red-100 text-red-400 border border-red-200 rounded-[1px] px-[0.65rem] py-[0.2rem] text-[0.75rem] font-semibold">Bloqueado</span>
                            @else
                                <span class="bg-green-100 text-green-600 border border-green-200 rounded-[1px] px-[0.65rem] py-[0.2rem] text-[0.75rem] font-semibold">Ativo</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 border-b border-fs-border text-[0.85rem]">
                            <div class="flex gap-2 flex-wrap">
                                <form method="POST" action="{{ route('admin.customers.toggle-block', $user->id) }}" class="inline"
                                ...
                                <form method="POST" action="{{ route('admin.customers.destroy', $user->id) }}" class="inline"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-fs-gray text-[0.9rem]">Nenhum cliente encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
