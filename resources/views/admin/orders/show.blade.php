...
@section('content')
    <div class="p-6" x-data="{
        showCancelModal: window.location.hash === '#cancel',
        cancelReason: ''
    }">
        ...
        <div class="mb-5">
            <a href="{{ route('admin.orders') }}"
                class="text-fs-gray text-[0.85rem] no-underline inline-flex items-center gap-[0.4rem] hover:text-fs-dark transition-colors">
            ...
            <div class="flex items-start justify-between flex-wrap gap-4">
                ...
                <div class="flex items-center gap-4 flex-wrap">
                    ...
                    @if ($order->status === 'pending')
                        <span
                            class="bg-yellow-50 text-yellow-400 border border-yellow-200 rounded-[1px] px-[0.85rem] py-[0.3rem] text-[0.8rem] font-semibold">Pendente</span>
                    @elseif ($order->status === 'closed')
                        <span
                            class="bg-green-50 text-green-600 border border-green-200 rounded-[1px] px-[0.85rem] py-[0.3rem] text-[0.8rem] font-semibold">Fechada</span>
                    @elseif ($order->status === 'canceled')
                        <span
                            class="bg-red-50 text-red-400 border border-red-200 rounded-[1px] px-[0.85rem] py-[0.3rem] text-[0.8rem] font-semibold">Cancelada</span>
                    @endif

                    {{-- Actions for pending --}}
                    @if ($order->isPending())
                        <form method="POST" action="{{ route('admin.orders.close', $order->id) }}" class="inline"
                            onsubmit="return confirm('Fechar esta encomenda?')">
                            @csrf
                            <button type="submit"
        ...
        <div class="grid gap-6 items-start grid-cols-[1fr_340px]">
            ...
            <div class="bg-white border border-fs-border rounded-[2px] overflow-hidden">
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-[#f9f8f6]">
                            <th
                                class="px-4 py-[0.65rem] text-left text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] border-b border-fs-border">
                                Imagem</th>
                            <th
                                class="px-4 py-[0.65rem] text-left text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] border-b border-fs-border">
                                Cor</th>
                            <th
                                ...
                                Tam.</th>
                            <th
                                class="px-4 py-[0.65rem] text-right text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] border-b border-fs-border">
                                Qtd.</th>
                            <th
                                class="px-4 py-[0.65rem] text-right text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] border-b border-fs-border">
                                Preço unit.</th>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-[0.65rem] border-b border-fs-border text-[0.85rem]">
                                    <div class="flex items-center gap-[0.4rem]">
                                        @if ($item->color_code)
                                            <span
                                                class="inline-block w-[14px] h-[14px] rounded-full border border-white/15 flex-shrink-0"
                                                style="background:{{ $item->color_code }};"></span>
                                        @endif
                                        <span
                                            class="text-fs-gray">{{ $item->color?->name ?? ($item->color_code ?? '—') }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-[0.65rem] border-b border-fs-border text-fs-gray text-[0.85rem]">
                                    {{ $item->size ?? '—' }}</td>
                                <td
                                    class="px-4 py-[0.65rem] border-b border-fs-border text-fs-dark text-[0.85rem] text-right">
                                    {{ $item->qty }}</td>
                                <td
                                    class="px-4 py-[0.65rem] border-b border-fs-border text-fs-gray text-[0.85rem] text-right">
                                    {{ number_format($item->unit_price, 2, ',', '.') }}€</td>
                                <td
                                    class="px-4 py-[0.65rem] border-b border-fs-border text-fs-dark text-[0.85rem] font-semibold text-right">
            ...
            <div class="bg-white border border-fs-border rounded-[2px] p-6">
                ...
                <div class="flex flex-col gap-[0.85rem]">
                    ...
                    <div>
                        <p class="text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] m-0 mb-1">Forma de
                            pagamento</p>
                    ...
                    @if ($order->isCanceled() && ($order->reason_for_cancellation ?? null))
                        <div class="bg-red-50 border border-red-200 rounded-[1px] p-[0.85rem] mt-1">
                            <p
                                class="text-red-400 text-[0.75rem] font-semibold uppercase tracking-[0.05em] m-0 mb-[0.3rem]">
        ...
        <div x-show="showCancelModal" x-cloak class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
            id="cancel">
            <div class="bg-white border border-fs-border rounded-[2px] p-8 w-full max-w-[480px]">
                ...
                <form method="POST" action="{{ route('admin.orders.cancel', $order->id) }}">
                    @csrf
                    <textarea name="reason" x-model="cancelReason" placeholder="Motivo do cancelamento..." rows="4"
                        class="w-full bg-white border border-fs-border text-fs-dark rounded-[1px] px-3 py-[0.65rem] text-[0.9rem] outline-none resize-y focus:border-fs-purple"></textarea>
                    @error('reason')
