    ...
    <div x-data="{
        ...
        <form method="GET" action="{{ route('admin.catalog') }}" class="flex items-center gap-3 mb-5 flex-wrap">
            ...
            <select name="category"
                class="bg-white border border-fs-border text-fs-dark rounded-[1px] px-3 py-[0.6rem] text-[0.9rem] outline-none">
            ...
            <button type="submit"
                class="bg-white border border-fs-border text-fs-gray rounded-[1px] px-4 py-[0.6rem] text-[0.85rem] cursor-pointer transition-colors duration-150 hover:border-fs-dark hover:text-fs-dark">
                Filtrar
            </button>

            <button type="button"
                @click="showModal = true; editingId = null; editName = ''; editDesc = ''; editCatId = '';"
                ...
                @foreach ($images as $img)
                    <div class="bg-white border border-fs-border rounded-[2px] overflow-hidden flex flex-col">
                        <div class="h-40 bg-fs-light flex items-center justify-center overflow-hidden">
                            @if ($img->catalogUrl)
                                <img src="{{ $img->catalogUrl }}" alt="{{ $img->name }}"
                        ...
                        <div class="p-3 flex-1 flex flex-col gap-[0.3rem]">
                            <div
                                class="text-fs-dark text-[0.9rem] font-semibold whitespace-nowrap overflow-hidden text-ellipsis">
                                {{ $img->name }}</div>
                            <div class="text-fs-gray text-[0.75rem]">{{ $img->category?->name ?? '—' }}</div>
                            @if ($img->description)
                                <div class="text-[#aaa] text-[0.75rem] whitespace-nowrap overflow-hidden text-ellipsis">
                                    {{ $img->description }}</div>
                            @endif
                            <div class="flex gap-2 mt-auto pt-2">
                                ...
                                <button type="button" @click="deleteId = {{ $img->id }}"
            ...
            <div class="flex justify-center gap-[0.4rem] py-4">
                ...
                @else
                    <a href="{{ $images->previousPageUrl() }}"
                        class="bg-white text-fs-gray border border-fs-border px-[0.9rem] py-[0.4rem] text-[0.8rem] rounded-[1px] no-underline transition-all duration-150 hover:border-fs-dark hover:text-fs-dark">←</a>
                @endif
        ...
        <div x-show="showModal" x-cloak
            ...
            <div
                class="bg-white border border-fs-border rounded-[2px] w-full max-w-[480px] p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div x-show="editingId">
                    @foreach ($images as $img)
                        <form x-show="editingId === {{ $img->id }}" method="POST"
                            action="{{ route('admin.catalog.update', $img->id) }}" enctype="multipart/form-data"
                            class="flex flex-col gap-4">
                            @csrf
                            <div>
                                <label class="text-fs-gray text-[0.8rem] block mb-[0.4rem]">Nome *</label>
                            ...
                            <div>
                                ...
                                <select name="category_id"
                                    ...
                                    <option value="">Sem categoria</option>
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat->id }}" @selected($img->category_id == $cat->id)>
                                            {{ $cat->name }}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        <div x-show="deleteId !== null" x-cloak
            ...
            <div class="bg-white border border-fs-border rounded-[2px] w-full max-w-[380px] p-6">
                ...
                <p class="text-fs-gray text-[0.875rem] m-0 mb-5">Tem a certeza que deseja eliminar esta imagem? Esta ação é
                    irreversível.</p>

                @foreach ($images as $img)
                    <form x-show="deleteId === {{ $img->id }}" method="POST"
                        action="{{ route('admin.catalog.destroy', $img->id) }}" class="flex gap-3">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
