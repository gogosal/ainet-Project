...
<div x-data="{
    ...
    <div class="flex justify-end mb-5">
        <button type="button"
            @click="showModal = true; editingId = null; editName = '';"
        ...
        <table class="w-full border-collapse">
            <thead>
                <tr class="border-b border-fs-border">
                    <th class="text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] px-4 py-3 text-left">Imagem</th>
                    <th class="text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] px-4 py-3 text-left">Nome</th>
                    <th class="text-fs-gray text-[0.75rem] font-semibold uppercase tracking-[0.05em] px-4 py-3 text-center">Imagens</th>
                ...
                @forelse($categories as $cat)
                    ...
                    <tr class="border-b border-fs-border">
                        <td class="px-4 py-3">
                            ...
                            @else
                                <div class="w-12 h-12 bg-white rounded-[1px] border border-fs-border flex items-center justify-center text-[#aaa]">🏷️</div>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-fs-dark text-[0.9rem] font-medium">{{ $cat->name }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="bg-[rgba(124,111,160,0.12)] text-fs-purple rounded-[1px] px-[0.6rem] py-[0.2rem] text-[0.78rem] font-semibold">
    ...
    <div x-show="showModal" x-cloak
        ...
        <div class="bg-white border border-fs-border rounded-[2px] w-full max-w-[420px] p-6">
            ...
            <div x-show="!editingId">
                <form method="POST" action="{{ route('admin.categories.store') }}" enctype="multipart/form-data" class="flex flex-col gap-4">
                    @csrf
                    <div>
            ...
            <div x-show="editingId">
                ...
                <form x-show="editingId === {{ $cat->id }}"
                      method="POST" action="{{ route('admin.categories.update', $cat->id) }}"
    ...
    <div x-show="deleteId !== null" x-cloak
        ...
        <div class="bg-white border border-fs-border rounded-[2px] w-full max-w-[380px] p-6">
            ...
            <p class="text-fs-gray text-[0.875rem] m-0 mb-5">Tem a certeza que deseja eliminar esta categoria? Esta ação é irreversível.</p>

            @foreach($categories as $cat)
            <form x-show="deleteId === {{ $cat->id }}"
                  method="POST" action="{{ route('admin.categories.destroy', $cat->id) }}"
                  class="flex gap-3">
                @csrf
                @method('DELETE')
                <button type="submit"
