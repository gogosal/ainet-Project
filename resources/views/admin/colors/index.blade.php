...
@section('content')
    <div x-data="{
        ...
        <div class="flex justify-end mb-5">
            <button type="button" @click="showModal = true; editingId = null; editName = ''; editCode = '';"
                        ...
                        <tr class="border-b border-fs-border">
                            <td class="px-4 py-3">
                                <div class="w-10 h-10 rounded-[1px] border border-[rgba(0,0,0,0.12)] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.2)]"
                                    style="background:#{{ $color->cleanCode }};"></div>
                            </td>
                            <td class="px-4 py-3 text-fs-dark text-[0.9rem] font-medium">{{ $color->name }}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="bg-white border border-fs-border text-fs-purple rounded-[1px] px-[0.5rem] py-[0.2rem] text-[0.78rem] font-mono">{{ $color->code }}</span>
                            </td>
                            <td class="px-4 py-3">
                                @if ($color->baseFile)
                                    <img src="/storage/tshirt_base/{{ $color->baseFile }}" alt="{{ $color->name }}"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex gap-2 justify-end">
                                    <button type="button"
                                        @click="showModal = true; editingId = '{{ $color->code }}'; editName = '{{ addslashes($color->name) }}'; editCode = '{{ $color->code }}';"
                                    ...
                                    <button type="button" @click="deleteId = '{{ $color->code }}'"
            ...
            <div class="bg-white border border-fs-border rounded-[2px] w-full max-w-[420px] p-6">
                ...
                <div x-show="!editingId">
                    <form method="POST" action="{{ route('admin.colors.store') }}" enctype="multipart/form-data"
                        ...
                        <div>
                            <label class="text-fs-gray text-[0.8rem] block mb-[0.4rem]">Imagem base da t-shirt
                                (opcional)</label>
                ...
                <div x-show="editingId">
                    @foreach ($colors as $color)
                        {{-- ATENÇÃO AQUI: usamos urlencode() para transformar o # em %23 --}}
                        <form x-show="editingId === '{{ $color->code }}'" method="POST"
                            action="{{ route('admin.colors.update', ['color' => urlencode($color->code)]) }}"
                            ...
                            <div>
                                ...
                                <div class="flex gap-2 items-center">
                                    ...
                                    <span
                                        class="bg-white border border-fs-border text-fs-gray rounded-[1px] px-3 py-[0.6rem] font-mono text-[0.9rem]">{{ $color->code }}</span>
                                </div>
                            </div>
                            <div>
                                <label class="text-fs-gray text-[0.8rem] block mb-[0.4rem]">Nome *</label>
        ...
        <div x-show="deleteId !== null" x-cloak
            ...
            <div class="bg-white border border-fs-border rounded-[2px] w-full max-w-[380px] p-6">
                ...
                <p class="text-fs-gray text-[0.875rem] m-0 mb-5">Tem a certeza que deseja eliminar esta cor? Esta ação é
                    irreversível.</p>

                @foreach ($colors as $color)
                    {{-- A CORREÇÃO ESTÁ AQUI: adicionar o urlencode() na rota destroy --}}
                    <form x-show="deleteId === '{{ $color->code }}'" method="POST"
                        action="{{ route('admin.colors.destroy', ['color' => urlencode($color->code)]) }}"
                        class="flex gap-3">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
