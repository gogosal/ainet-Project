...
<div class="mb-[1.75rem]">
    ...
    <h1 class="text-[1.6rem] font-light tracking-[-0.02em] text-fs-dark m-0">Escolhe o teu <em class="italic font-bold">design.</em></h1>
</div>
...
{{-- Two-column layout: 220px sidebar + 1fr main --}}
<div class="grid gap-6 items-start [grid-template-columns:220px_1fr]">
    ...
    <aside class="bg-fs-bg border border-fs-border rounded-[2px] sticky top-6">
        ...
        <div class="p-[1.1rem]">
            ...
            {{-- Search --}}
            <div class="mb-[1.4rem]">
                <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.55rem]">Pesquisa</label>
            ...
            <div x-data="{ open: false }" class="mb-[1.4rem]">
                <button @click="open = !open"
                    ...
                    <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted cursor-pointer m-0">Categoria</label>
                    <svg :class="open ? 'rotate-180' : ''" class="transition-transform duration-200 shrink-0" width="12" height="12" viewBox="0 0 12 12" fill="none">
                ...
                <div x-show="open" x-collapse class="flex flex-col gap-[0.15rem]">
                    <a href="{{ route('catalog', array_filter(['search' => $search])) }}"
                       class="flex items-center gap-2 w-full px-2 py-[0.4rem] cursor-pointer text-left rounded-[1px] no-underline transition-colors duration-100 hover:bg-black/[0.04]">
                        <span class="w-3 h-3 rounded-full shrink-0 border-[1.5px] {{ $categoryId === null ? 'border-fs-purple bg-fs-purple' : 'border-fs-mid bg-transparent' }}"></span>
                        <span class="text-[0.8rem] {{ $categoryId === null ? 'text-fs-purple font-semibold' : 'text-fs-gray font-normal' }}">Todos</span>
                    </a>

                    @foreach($categories as $cat)
                        <a href="{{ route('catalog', array_filter(['search' => $search, 'category' => $cat->id])) }}"
                           class="flex items-center gap-2 w-full px-2 py-[0.4rem] cursor-pointer text-left rounded-[1px] no-underline transition-colors duration-100 hover:bg-black/[0.04]">
                            <span class="w-3 h-3 rounded-full shrink-0 border-[1.5px] {{ (int)$categoryId === $cat->id ? 'border-fs-purple bg-fs-purple' : 'border-fs-mid bg-transparent' }}"></span>
                            <span class="text-[0.8rem] {{ (int)$categoryId === $cat->id ? 'text-fs-purple font-semibold' : 'text-fs-gray font-normal' }}">{{ $cat->name }}</span>
                        </a>
                    @endforeach

                    <a href="{{ route('catalog', array_filter(['search' => $search, 'category' => -1])) }}"
                       class="flex items-center gap-2 w-full px-2 py-[0.4rem] cursor-pointer text-left rounded-[1px] no-underline transition-colors duration-100 hover:bg-black/[0.04]">
                        <span class="w-3 h-3 rounded-full shrink-0 border-[1.5px] {{ $categoryId === '-1' ? 'border-fs-purple bg-fs-purple' : 'border-fs-mid bg-transparent' }}"></span>
                        <span class="text-[0.8rem] {{ $categoryId === '-1' ? 'text-fs-purple font-semibold' : 'text-fs-gray font-normal' }}">Sem categoria</span>
                    </a>
                </div>
            </div>
            ...
            <div x-data="{ open: false }" class="mb-[1.4rem]">
                <button @click="open = !open"
                        class="flex items-center justify-between w-full bg-transparent border-0 p-0 cursor-pointer mb-[0.65rem]">
                    <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted cursor-pointer m-0">Cores</label>
                    <svg :class="open ? 'rotate-180' : ''" class="transition-transform duration-200 shrink-0" width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M2 4.5L6 8.5L10 4.5" stroke="#b8b4ae" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </button>
                    ...
                    <div class="flex flex-wrap gap-[0.35rem] mb-2">
                        @foreach($colors as $color)
                            <div title="{{ $color->name }}"
                                 class="w-5 h-5 rounded-full border border-black/10 cursor-default transition-transform duration-150 hover:scale-110"
            ...
            {{-- Price --}}
            <div>
                <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.55rem]">Preço</label>
                <div class="px-[0.9rem] py-[0.7rem] bg-fs-light border border-fs-border rounded-[1px]">
                    <div class="flex justify-between items-baseline mb-[0.3rem]">
                        ...
                        <span class="text-fs-dark font-bold text-[0.95rem]">€{{ number_format($prices->unit_price_catalog, 2) }}</span>
                    </div>
                    <div class="text-fs-muted text-[0.68rem] leading-[1.4]">
                        Desc. a partir de {{ $prices->qty_discount }} un.
                        <span class="text-fs-green block mt-[2px]">→ €{{ number_format($prices->unit_price_catalog_discount, 2) }}/un</span>
                    </div>
                </div>
            </div>
        ...
        @else
            <div class="grid grid-cols-3 gap-4 mb-8">
                @foreach($images as $image)
                    ...
                    <div class="bg-white border border-fs-border rounded-[2px] overflow-hidden transition-colors duration-150 hover:border-fs-purple">

                        <div class="bg-fs-bg h-[180px] flex items-center justify-center border-b border-fs-border overflow-hidden p-4">
                        ...
                        <div class="p-[0.85rem]">
                            @if($image->category)
                                <span class="inline-block text-[0.6rem] font-bold tracking-[0.1em] uppercase text-fs-purple mb-[0.35rem]">{{ $image->category->name }}</span>
                            @endif
                            <h3 class="text-fs-dark text-[0.85rem] font-semibold m-0 mb-[0.2rem] whitespace-nowrap overflow-hidden text-ellipsis">
                                {{ $image->name }}
                            </h3>
                            ...
                            <div class="flex items-center justify-between mt-2">
                                <span class="text-fs-dark font-bold text-[0.9rem]">€{{ number_format($prices->unit_price_catalog, 2) }}</span>
                                <div class="flex gap-[0.35rem]">
                                    ...
                                    <button type="button"
                                            @click="openModal({{ $image->id }}, '{{ addslashes($image->name) }}', '{{ $thumbDesignUrl }}', '{{ $colors->first()?->code ?? '' }}')"
...
<div x-show="showModal"
     class="fixed inset-0 bg-[rgba(26,26,26,0.4)] z-[200] flex items-center justify-center p-4"
    ...
    <div class="bg-fs-light border border-fs-border rounded-[2px] w-full max-w-[460px] overflow-hidden">
        ...
        <div class="flex items-center justify-between px-[1.35rem] py-[1.1rem] border-b border-fs-border">
            <div>
                <div class="text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-[0.2rem]">Adicionar ao carrinho</div>
        ...
        <form method="POST" action="{{ route('cart.store') }}">
            ...
            <div class="px-[1.35rem] py-[1.35rem] flex gap-[1.35rem]">
                ...
                <div class="shrink-0">
                    <x-tshirt-preview
                        :colorCode="$colors->first()?->code ?? 'white'"
                        :imageUrl="null"
                        size="110px" />
                </div>
                ...
                <div class="flex-1 flex flex-col gap-4">
                    {{-- Color --}}
                    <div>
                        <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-2">Cor</label>
                    ...
                    {{-- Size --}}
                    <div>
                        <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-2">Tamanho</label>
                        <div class="flex gap-[0.35rem]">
                    ...
                    {{-- Qty --}}
                    <div>
                        <label class="block text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted mb-2">Quantidade</label>
                        <div class="flex items-center gap-2">
                            <button type="button" @click="qty = Math.max(1, qty - 1)"
                                    class="bg-transparent border border-[#d8d5d0] w-[30px] h-[30px] text-fs-gray cursor-pointer text-[1rem] flex items-center justify-content-center rounded-[1px] transition-all duration-150 font-[inherit] hover:border-fs-dark flex justify-center">−</button>
                            <span class="text-fs-dark font-semibold min-w-[2rem] text-center text-[0.9rem]" x-text="qty"></span>
                    ...
                    <div class="bg-fs-bg border border-fs-border px-3 py-[0.6rem] text-[0.75rem] rounded-[1px]">
                        <div class="flex justify-between text-fs-gray">
                            <span>Por unidade</span>
                            <span class="text-fs-dark font-bold">€{{ number_format($prices->unit_price_catalog, 2) }}</span>
                        </div>
                        <div x-show="qty >= {{ $prices->qty_discount }}" class="flex justify-between text-fs-green mt-1 text-[0.7rem]">
                            <span>✓ Desconto de quantidade</span>
                            <span>€{{ number_format($prices->unit_price_catalog_discount, 2) }}/un</span>
                        </div>
                        <div x-show="qty < {{ $prices->qty_discount }}" class="text-[#c8c4be] mt-1 text-[0.68rem]">
