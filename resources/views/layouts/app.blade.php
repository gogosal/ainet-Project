        ...
        <div style="max-width:1280px;margin:0 auto;padding:0 2rem;height:56px;display:flex;align-items:center;gap:2rem;">
            ...
            <nav style="display:flex;gap:1.75rem;margin-left:.5rem;">
                <a href="{{ route('catalog') }}"
                    ...
                    onmouseout="this.style.color='{{ request()->routeIs('catalog') ? '#7c6fa0' : '#aaa8a3' }}'">
                    Catálogo
                </a>
                ...
                <a href="{{ route('try-on') }}"
                    ...
                    onmouseover="this.style.color='#1a1a1a'"
                    onmouseout="this.style.color='{{ request()->routeIs('try-on') ? '#7c6fa0' : '#aaa8a3' }}'">
                    Provador 3D
                </a>
                ...
                @auth
                    ...
                    <a href="{{ $ordersRoute }}"
                        ...
                        onmouseover="this.style.color='#1a1a1a'"
                        onmouseout="this.style.color='{{ $ordersActive ? '#7c6fa0' : '#aaa8a3' }}'">
                        Encomendas
                    </a>
                    ...
                    @if (auth()->user()->isAdmin() || auth()->user()->isEmployee())
                        <a href="{{ route('my-images') }}"
                            ...
                            onmouseover="this.style.color='#1a1a1a'"
                            onmouseout="this.style.color='{{ request()->routeIs('my-images') ? '#7c6fa0' : '#aaa8a3' }}'">
                            Imagens
                        </a>
                    @endif
                @endauth
                    ...
                    <div x-data="{ open: false }" style="position:relative;">
                        <button @click="open = !open"
                            ...
                            @if (auth()->user()->photo_url)
                                ...
                                <span
                                    style="display:none;width:20px;height:20px;border-radius:50%;background:#1a1a1a;align-items:center;justify-content:center;font-size:.62rem;color:#f5f4f1;font-weight:700;">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                            @else
                                <span
                                    style="width:20px;height:20px;border-radius:50%;background:#1a1a1a;display:inline-flex;align-items:center;justify-content:center;font-size:.62rem;color:#f5f4f1;font-weight:700;">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                            @endif
                            <span style="font-weight:500;">{{ explode(' ', auth()->user()->name)[0] }}</span>
                            <svg style="width:10px;height:10px;color:#aaa" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            @if (auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}"
                                    style="display:block;padding:.45rem .7rem;color:#888;text-decoration:none;font-size:.75rem;font-weight:500;letter-spacing:.04em;transition:color .15s;"
                                    onmouseover="this.style.color='#1a1a1a'"
                                    onmouseout="this.style.color='#888'">Dashboard</a>
                            @elseif(auth()->user()->isEmployee())
                                <a href="{{ route('employee.orders') }}"
                                    style="display:block;padding:.45rem .7rem;color:#888;text-decoration:none;font-size:.75rem;font-weight:500;letter-spacing:.04em;transition:color .15s;"
                                    onmouseover="this.style.color='#1a1a1a'"
                                    onmouseout="this.style.color='#888'">Encomendas</a>
                            @endif
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <button type="submit"
                                    ...
                                    onmouseover="this.style.color='#922b21'"
                                    onmouseout="this.style.color='#c0392b'">Logout</button>
                            </form>
                        </div>
                    </div>
                @else
