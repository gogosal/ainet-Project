...
@section('content')
    <div style="display:flex;flex-direction:column;height:calc(100vh - 56px - 5rem);" x-data="{
        ...
        <div
            ...
            <div class="bg-fs-bg border border-fs-border rounded-[2px] overflow-hidden flex flex-col min-h-0 h-full">
                ...
                <div data-lenis-prevent
                    ...
                    @foreach ($designs as $design)
                        ...
                        @endphp
                        <button type="button" @click="selectedDesignId = {{ $design->id }}"
                            :class="selectedDesignId === {{ $design->id }} ? 'bg-white border-fs-purple' :
                                'bg-transparent border-transparent hover:bg-black/[0.04]'"
                            ...
                            <div
                                class="w-12 h-12 shrink-0 bg-fs-light rounded-[1px] overflow-hidden border border-fs-border">
                            ...
                            <div class="min-w-0 flex-1">
                                <p :class="selectedDesignId === {{ $design->id }} ? 'text-fs-dark font-semibold' :
                                    'text-fs-gray font-normal'"
            ...
            {{-- CENTER: 3D Viewport --}}
            <div
                class="bg-fs-bg border border-fs-border rounded-[2px] relative overflow-hidden min-h-0 shadow-[inset_0_2px_20px_rgba(0,0,0,0.06)]">
            ...
            <div class="bg-fs-bg border border-fs-border rounded-[2px] p-[0.85rem] flex flex-col gap-3">
                ...
                @if ($selectedImage)
                    <div class="border-b border-fs-border pb-[0.6rem]">
                        <p class="text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted m-0 mb-[0.2rem]">Design
                        </p>
                        <p
                            class="text-fs-dark text-[0.84rem] font-semibold m-0 whitespace-nowrap overflow-hidden text-ellipsis">
                ...
                {{-- Side toggle --}}
                <div>
                    <p class="text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted m-0 mb-2">Posição da
                        estampa</p>
                    ...
                    <div class="flex gap-[0.35rem] flex-wrap">
                        @foreach ($colors as $color)
                            <button type="button" @click="selectedColor = '{{ $color->code }}'"
                                title="{{ $color->name }}"
                                :class="selectedColor === '{{ $color->code }}' ? 'ring-2 ring-fs-purple ring-offset-1' :
                ...
                {{-- Size --}}
                <div>
                    <p class="text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted m-0 mb-[0.4rem]">Tamanho
                    </p>
                    <div class="flex gap-[0.3rem] flex-wrap">
                        @foreach (['XS', 'S', 'M', 'L', 'XL'] as $size)
                            <button type="button" @click="selectedSize = '{{ $size }}'"
                                :class="selectedSize === '{{ $size }}' ? 'bg-fs-dark text-fs-light border-fs-dark' :
                ...
                {{-- Qty --}}
                <div>
                    <p class="text-[0.58rem] font-bold tracking-[0.14em] uppercase text-fs-muted m-0 mb-[0.4rem]">Quantidade
                    </p>
                    <div class="flex items-center gap-[0.4rem]">
                        ...
                        <span class="text-fs-dark font-semibold min-w-[1.8rem] text-center text-[0.88rem]"
                            x-text="qty"></span>
                ...
                @if ($prices)
                    <div class="bg-fs-light border border-fs-border px-3 py-[0.6rem] text-[0.75rem] rounded-[1px]">
                        <div class="flex justify-between text-fs-gray mb-[0.2rem]">
                            ...
                            <span
                                class="text-fs-dark font-bold">€{{ number_format($prices->unit_price_catalog, 2) }}</span>
                        </div>
                        ...
                        <div class="flex justify-between text-fs-dark font-bold text-[0.9rem]">
                            <span>Total</span>
                            <span
                                x-text="'€' + ((qty >= {{ $prices->qty_discount }} ? {{ $prices->unit_price_catalog_discount }} : {{ $prices->unit_price_catalog }}) * qty).toFixed(2)"></span>
                        </div>
                    </div>
                @endif

                {{-- Add to cart form --}}
                <form method="POST" action="{{ route('view3d.cart') }}" class="mt-auto">
                    @csrf
